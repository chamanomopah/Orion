...
PAI uses **relative paths** for all hook commands to ensure compatibility across Windows, macOS, and Linux. Hook commands in `settings.json` use paths relative to the project root (where `settings.json` is located).

### ✅ Correct (Cross-Platform)
```json
{ "command": "bun .claude/hooks/StartupGreeting.hook.ts" }
```
...
The `${PAI_DIR}` variable does **not** work on Windows because:
- Windows CMD uses `%PAI_DIR%` syntax, not `${PAI_DIR}`
- Claude Code executes hooks directly without a shell on Windows
- No environment variable expansion occurs in `command` fields

**Relative paths work on all platforms** because:
- Claude Code executes hooks from the project root directory
- `.claude/hooks/` resolves correctly on Windows, macOS, and Linux
- No shell-specific features required

### Windows-Specific Notes

1. **Prerequisites on Windows**:
   - Install Bun from [bun.sh](https://bun.sh) (Windows installer available)
   - Install Claude Code with `npm install -g @anthropic-ai/claude-code`
   - Git Bash or WSL recommended (for Unix commands in skills)

2. **Installation Wizard on Windows**:
   - The `INSTALL.ts` script now detects Windows and adapts automatically
   - **Permissions**: Skips `chmod`/`chown` (Windows uses ACLs)
   - **Voice Server**: Uses `netstat`/`taskkill` instead of `lsof`/`kill`
   - **Bun Install**: Provides Windows-specific download instructions
   - **Alias**: Creates `pai.bat` instead of modifying `.zshrc`

3. **Limitations on Windows**:
   - **Voice Server**: Requires Git Bash or WSL to run the start script
   - **Statusline Feature**: Uses a Bash script (`.claude/statusline-command.sh`) which requires:
     - Git Bash
     - WSL (Windows Subsystem for Linux)
     - Or disable by removing `"statusLine"` from `settings.json`
   - **Some Skills**: May use Unix-specific commands (grep, find, sed)

4. **Manual Alias Setup (if wizard fails)**:
   ```batch
   # Create C:\Users\YourName\pai.bat with:
   @echo off
   bun %USERPROFILE%\.claude\skills\PAI\Tools\pai.ts %*
   ```

5. **File Permissions**: Windows doesn't use Unix execute permissions (`chmod +x`). Hooks work via file extensions and PATHEXT.

6. **Best Experience on Windows**:
   - Use **Git Bash** for terminal (provides Unix commands)
   - Or use **WSL2** for full Unix compatibility
   - Native PowerShell works but with some feature limitations
