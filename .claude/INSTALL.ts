...
function fixPermissions(targetDir: string): void {
  ...
  if (IS_WINDOWS) {
    ...
    // Files created by user are already owned by user
    printInfo('Windows detected - skipping chmod/chown (uses ACLs)');
    printInfo('Files are already accessible to current user');
    return;
  }
...
function findRunningVoiceServers(): number[] {
  try {
    if (IS_WINDOWS) {
      // Windows: use netstat to find processes listening on port
      const result = execSync(`netstat -ano | findstr :${VOICE_SERVER_PORT}`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
      // netstat output format: protocol | local address | foreign address | state | PID
...
async function startVoiceServer(): Promise<boolean> {
  ...
  if (IS_WINDOWS) {
    ...
    try {
      ...
      printError('Bash not found on Windows');
      printInfo('Voice server requires Git Bash or WSL');
      printInfo('Install Git Bash: https://git-scm.com/download/win');
      printInfo('Or use WSL: wsl --install');
      return false;
    }
  }
  ...
  try {
    const child = spawn('bash', [startScript], {
      ...
      shell: IS_WINDOWS, // On Windows, use shell to find bash
    });
    child.unref();
...
function installBun(): void {
  ...
  try {
    ...
    if (IS_WINDOWS) {
      printError('Bun installation script is Unix-only');
      printInfo('Download Windows installer from: https://bun.sh/install');
      printInfo('Or use: powershell -c "irm bun.sh/install.ps1|iex"');
      printInfo('After installation, restart this script');
      process.exit(1);
    }
    ...
    try {
      execSync('curl -fsSL https://bun.sh/install | bash', {
        stdio: 'inherit',
        shell: IS_MACOS || IS_LINUX ? '/bin/bash' : undefined,
      });
...
function setupAlias(): void {
  const aliasCommand = `bun ~/.claude/skills/PAI/Tools/pai.ts`;
  const marker = '# PAI alias';

  if (IS_WINDOWS) {
    // Windows: Create batch file
    const batchPath = join(HOME, 'pai.bat');
    try {
      const batchContent = `@echo off\r\nbun %USERPROFILE%\\.claude\\skills\\PAI\\Tools\\pai.ts %*\r\n`;
      writeFileSync(batchPath, batchContent);
      printSuccess('Created pai.bat in user directory');
      printInfo('You can now run "pai" from Command Prompt or PowerShell');
    } catch (err: any) {
      printWarning(`Could not create pai.bat: ${err.message}`);
    }
    return;
  }

  // Unix-like: Update shell config
  ...
  if (IS_MACOS) {
    ...
    // Linux: default to .bashrc (more common than zsh)
    shellConfig = join(HOME, '.bashrc');
    shellName = '.bashrc';
  }

  const aliasLine = `alias pai='${aliasCommand}'`;

  try {
    let content = existsSync(shellConfig) ? readFileSync(shellConfig, 'utf-8') : '';

    if (content.includes('alias pai=')) {
      // Update existing
      content = content.replace(/alias pai=.*/, aliasLine);
      writeFileSync(shellConfig, content);
      printSuccess(`Updated PAI alias in ${shellName}`);
    } else {
      // Add new
      writeFileSync(shellConfig, content + `\n${marker}\n${aliasLine}\n`);
      printSuccess(`Added PAI alias to ${shellName}`);
    }
    printInfo(`Run "source ~/${shellName}" to enable "pai" command`);
  } catch (err: any) {
    printWarning(`Could not update ${shellName}: ${err.message}`);
  }
}
...
function generateSettingsJson(config: InstallConfig): object {
  ...
  return {
    ...
    "hooks": {
      "PreToolUse": [
        { "matcher": "Bash", "hooks": [{ "type": "command", "command": "bun .claude/hooks/SecurityValidator.hook.ts" }] },
        { "matcher": "Edit", "hooks": [{ "type": "command", "command": "bun .claude/hooks/SecurityValidator.hook.ts" }] },
        { "matcher": "Write", "hooks": [{ "type": "command", "command": "bun .claude/hooks/SecurityValidator.hook.ts" }] },
        { "matcher": "Read", "hooks": [{ "type": "command", "command": "bun .claude/hooks/SecurityValidator.hook.ts" }] },
        { "matcher": "AskUserQuestion", "hooks": [{ "type": "command", "command": "bun .claude/hooks/SetQuestionTab.hook.ts" }] }
      ],
      "PostToolUse": [
        { "matcher": "AskUserQuestion", "hooks": [{ "type": "command", "command": "bun .claude/hooks/QuestionAnswered.hook.ts" }] }
      ],
      "SessionEnd": [
        { "hooks": [
          { "type": "command", "command": "bun .claude/hooks/WorkCompletionLearning.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/SessionSummary.hook.ts" }
        ]}
      ],
      "UserPromptSubmit": [
        { "hooks": [
          { "type": "command", "command": "bun .claude/hooks/FormatReminder.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/AutoWorkCreation.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/ExplicitRatingCapture.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/ImplicitSentimentCapture.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/UpdateTabTitle.hook.ts" }
        ]}
      ],
      "SessionStart": [
        { "hooks": [
          { "type": "command", "command": "bun .claude/hooks/StartupGreeting.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/LoadContext.hook.ts" },
          { "type": "command", "command": "bun .claude/hooks/CheckVersion.hook.ts" }
        ]}
      ],
      "Stop": [{ "hooks": [{ "type": "command", "command": "bun .claude/hooks/StopOrchestrator.hook.ts" }] }],
      "SubagentStop": [{ "hooks": [{ "type": "command", "command": "bun .claude/hooks/AgentOutputCapture.hook.ts" }] }]
    },
...
async function main(): Promise<void> {
  ...
  if (IS_WINDOWS) {
    print('');
    print(`${c.bold}Windows Platform Notes${c.reset}`);
    print(`${c.gray}─────────────────────────────────────────────────${c.reset}`);
    printInfo('Voice server requires Git Bash or WSL');
    printInfo('Some features may have limited functionality on Windows');
    printInfo('For best experience, consider using WSL2');

    // Check if bash is available
    try {
      execSync('bash --version', { stdio: 'pipe' });
      printSuccess('Bash found - voice server can run');
    } catch {
      printWarning('Bash not found - voice server unavailable');
      printInfo('Install Git Bash: https://git-scm.com/download/win');
    }
  }
  ...
  if (passed) {
    ...
    if (IS_WINDOWS) {
      print(`  ${c.cyan}Next:${c.reset} Restart your terminal, then run ${c.green}pai${c.reset}`);
      printInfo('The "pai" command is available as pai.bat');
    } else {
      const shellFile = IS_MACOS ? '~/.zshrc' : '~/.bashrc';
      print(`  ${c.cyan}Next:${c.reset} Run ${c.green}source ${shellFile}${c.reset} then ${c.green}pai${c.reset}`);
    }
  } else {
