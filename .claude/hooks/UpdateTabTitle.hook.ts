...
const SYSTEM_PROMPT = `Create a 3-5 word COMPLETE SENTENCE summarizing the user's CURRENT MESSAGE.

CRITICAL: Summarize ONLY the current message. If previous messages discuss a different topic, IGNORE THEM COMPLETELY. The current message is what matters.
...
- "Drafting manager response letter."
...
- "Completing the task." (generic - WHAT task?)
...
async function summarizePrompt(prompt: string, transcriptPath: string): Promise<SummaryResult> {
  // Only use context for very short prompts that need disambiguation
  // Long prompts (50+ chars) are self-explanatory — context just contaminates
  const promptLength = prompt.trim().length;
  const needsContext = promptLength < 50;
  const context = needsContext ? getRecentContext(transcriptPath, 1) : '';

  // Build user prompt — current message is ALWAYS the focus
  // For short prompts: minimal context helps disambiguate ("fix it" → fix what?)
  // For long prompts: no context needed, the message speaks for itself
  let userPrompt: string;
  if (context) {
    // Put context FIRST, current message LAST (recency bias helps the model focus on it)
    userPrompt = `Previous context (ONLY use if current message is ambiguous):
${context}

CURRENT MESSAGE — summarize THIS and ONLY this:
${prompt.slice(0, 800)}`;
  } else {
...
/**
 * Announce prompt receipt via voice server (ElevenLabs TTS).
 * Summary is already a complete sentence from inference.
 */
async function announceVoice(summary: string): Promise<void> {
  const identity = await import('./lib/identity').then(m => m.getIdentity());

  // Summary is already a complete sentence (e.g., "Fixing the bug now.")
...
async function main() {
  try {
    ...
    // Reject summaries that are entirely generic (e.g., "Completing the task.")
    // but allow specific uses like "Fixing task handler." where "task" is part of a specific noun
    const GENERIC_PHRASES = [
      /^completing (the )?task\.?$/i,
      /^working on (the )?(task|request|work)\.?$/i,
      /^processing (the )?(request|task|work)\.?$/i,
      /^handling (the )?(request|task|work)\.?$/i,
      /^finishing (the )?(task|work)\.?$/i,
      /task completion/i,
    ];
